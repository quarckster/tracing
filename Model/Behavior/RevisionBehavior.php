...
class RevisionBehavior extends ModelBehavior {
	...
	public function diff(&$Model, $from_version_id = null, $to_version_id = null, $options = array()) {
		...
		foreach ($keys as $field) {
			$all_values = Set::extract('/'.$Model->alias.'/'.$field, $all);
	...
	public function revisions(&$Model, $options = array(), $include_current = false) {
		...
		$data = $this->specialAfterFind($Model->ShadowModel->find('all',$options));
		
		//return $Model->ShadowModel->find('all',$options);
		return $data;
	}
	
	public function specialAfterFind($results) {
		...
		if (isset($val['Incident']['version_created'])) {
		    $timestamp = strtotime($results[$key]['Incident']['version_created']);
		    $results[$key]['Incident']['version_created'] = date('d.m.Y H:i', $timestamp);
		}
	    }
	    return $results;
	}
	...
	public function afterSave(&$Model, $created) {
		...
		if ($created) {
			...
			$changed_by = AuthComponent::user('displayname');
			$Model->ShadowModel->set('changed_by', $changed_by);
			$Model->ShadowModel->set('version_created',date('Y-m-d H:i:s'));
