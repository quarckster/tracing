...
<h2>Входящее. Версия от <?php echo $history['Incident']['version_created']; ?></h2>
	<dl><?php $i = 0; $class = ' class="altrow"';?>
		<!--<dt<?php if ($i % 2 == 0) echo $class;?>>Зарегистрировал</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['user_sid']; ?>
			&nbsp;
		</dd>-->
		<dt<?php if ($i % 2 == 0) echo $class;?>>Редактировал</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['changed_by']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>>Дата регистрации</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['start_date']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>>Дата выполнения</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['exp_date']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>>Входящий номер</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['incoming_num']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>>Организация</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['organization']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>>Содержание</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['content']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>>Номер ТО</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php echo $history['Incident']['number_to']; ?>
			&nbsp;
		</dd>
		<dt<?php if ($i % 2 == 0) echo $class;?>>Уведомлены</dt>
		<dd<?php if ($i++ % 2 == 0) echo $class;?>>
			<?php $notified = array();
			foreach ($history['Detail'] as $detail0):
...
<div class="actions">
	...
	<h3>Действия</h3>
	<ul>
		<li><?php echo $this->Html->link(__('Редактировать', true), array('action' => 'edit', $history['Incident']['id'])); ?> </li>
		<li><?php echo $this->Html->link(__('История изменений', true), array('action' => 'history', $history['Incident']['id'])); ?> </li>
		<li><?php echo $this->Html->link(__('Удалить', true), array('action' => 'delete', $history['Incident']['id']), null, sprintf(__('Вы уверены, что хотите удалить это входящее?', true))); ?> </li>
